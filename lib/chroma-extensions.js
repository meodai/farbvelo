...
chroma.okhsl = function(...args) {
    let h_deg, s_norm, l_norm, alpha = 1;

    if (args.length === 1 && typeof args[0] === 'object' && args[0] !== null) {
        if (Array.isArray(args[0])) { // Input: [h, s, l, alpha_optional]
            h_deg = args[0][0];
            s_norm = args[0][1];
            l_norm = args[0][2];
            alpha = (args[0][3] === undefined) ? 1 : args[0][3];
        } else { // Input: {h, s, l, alpha_optional}
            h_deg = args[0].h;
            s_norm = args[0].s;
            l_norm = args[0].l;
            alpha = (args[0].alpha === undefined) ? 1 : args[0].alpha;
        }
    } else if (args.length >= 3) { // Input: h, s, l, alpha_optional
        h_deg = args[0];
        s_norm = args[1];
        l_norm = args[2];
        alpha = (args[3] === undefined) ? 1 : args[3];
    } else {
        throw new Error('Invalid arguments for chroma.okhsl. Expected (h,s,l,[alpha]), or ([h,s,l,alpha]), or ({h,s,l,alpha}).');
    }

    if (typeof h_deg === 'undefined' || typeof s_norm === 'undefined' || typeof l_norm === 'undefined') {
         throw new Error('Invalid Okhsl components: h, s, and l must be provided.');
    }

    const oklabColor = okhslToOklab(h_deg, s_norm, l_norm);
    return chroma.oklab(oklabColor.L, oklabColor.a, oklabColor.b).alpha(alpha);
};
